...
# methy_df = pd.read_csv('../data/GDC_methy_pf_new.csv', index_col=0)


file_path = "/home/SENSETIME/chenfeiyang/data1/GDC_data/Gistic2_CopyNumber_Gistic2_all_thresholded.by_genes"
data = pd.read_csv(file_path, sep="\t", index_col=0)  # 3175929 rows × 11 column
methy_df = data.T
...
methy_df["sample"] = methy_df.index.str[:]
...
features = [c for c in methy_data.columns if
            c not in ["sample", "cancer_type", "split_type"] and c in df_p["cg_name"].values]
from sklearn.ensemble import RandomForestClassifier
...
fea_imp.sort_values(by="imp", inplace=True)
df_fea_acc = pd.DataFrame()
fea_num = []
fea_acc = []
for n in range(len(features))[8000:8010]:
    if n % 1 == 0:
        fea_num.append(n)
...
methy = methy_df[fea_imp["fea"].values[:8000]]
print(methy.shape)
## 22140
methy.to_csv(r"/home/SENSETIME/chenfeiyang/data1/data/CNV.csv")
print("Done")
############################################### methy end ##########################################
del methy_df
